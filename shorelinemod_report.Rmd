...
Eelgrass (*Z. marina*) is the primary subtidal and intertidal meadow-forming seagrass in British Columbia. Meadows may also consist of the non-native seagrass, *Zostera japonica*, in the intertidal zone. Seagrass occurs to depths of up to 10 meters and can form meadows many km^2^ in size [@Murphy2021a]. We used a spatial dataset of seagrass in the Salish Sea compiled in 2018 for @Cristiani2021. This dataset consists of surveyed and modeled data from a variety of government and non-governmental sources - primarily the British Columbia Marine Conservation Atlas (BCMCA 2011) and the ShoreZone coastal mapping inventory [@coastalandoceanresources2017] (Table \@ref(tab:eelgrasstable)). Shoreline data are linear features generated from aerial imagery that classify the ecological features of the BC coast including eelgrass. In @CEC2016, these linear features were combined with bathymetric data to generate predictions of eelgrass extent [@reshitnyk2016]. The dataset includes 685 spatially distinct meadows across the Strait of Georgia Bioregion as well as in the southern portions of the Northern Shelf Bioregion and Southern Shelf Bioregion (Figure \@ref(fig:studyareafig)).

Due to the combination of data with varying collection methodologies and error, there is uncertainty in the accuracy of our dataset, and it is best used for coarse estimates of presence and extent. Since the time of our analysis, there has been additional eelgrass mapping and refining of the existing datasets. A comparison to a new DFO dataset shows areas where our dataset may overpredict presence and where additional meadows have been added. In some cases, the polygons of the newer dataset are more precise (i.e., multiple small polygons that are represented as one large general polygon in the earlier dataset), however, these locations would not alter our analysis of shoreline modification. More significant, are areas where the datasets do not overlap. Our dataset generally has more polygons in the Discovery Islands, Lasqueti Island, Howe Sound, and the Gulf Islands, whereas the newer dataset fills data gaps in areas such as Sechelt Inlet and Saanich Inlet (Figure \@ref(fig:meadowDiff)). A newer ShoreZone dataset fills gaps along the Sunshine Coast, Texada Island, and near Vancouver (Figure \@ref(fig:meadowDiffShorezone)).

```{r eelgrasstable}
df <- data.frame(
  Source = c("British Columbia Marine Conservation Atlas", "Hakai Institue / Shorezone", "Islands Trust", "Community Mapping Network", "Personal observations"),
  Coverage = c("coastwide", "coastwide (with gaps in the Strait of Georgia)", "Gulf Islands and Howe Sound", "coastwide (but used just Tsawwassen and Boundary Bay polygons)", "West and North Vancouver"),
  Dates = c("1890-2010", "1992-2017", "2012-2014", "unknown", "2016"),
  Access = c("bcmca.ca", "catalogue.hakai.org", "islandstrust.bc.ca (restricted)", "cmnbc.ca/atlasgallery/eelgrass-bed-mapping", "request from author")
  )
...
Previous guidelines on the width of marine riparian buffer zones for protecting sensitive habitat typically range from 50-150 meters [@Lemieux2004; @Levings2001]. We followed the methodology in a similar study of anthropogenic impacts [@Iacarella2018] and examined shoreline modifications within 100 m of the coastline adjacent to each seagrass meadow. For our analysis we used the high water coastline from the 1:20,000 scale Freshwater Atlas provincial dataset [@geobc2010]. Quantifying modifications within a buffer zone requires generating consistent buffers from all meadows onto land. The perimeters of meadows, however, do not always exactly border the shoreline due to variable mapping accuracies and errors, as well as some meadows only existing in the subtidal zone. This would result in slightly different buffer extents on to land. Aside from a few exceptions, the majority of meadows are in close proximity to a coastline, and therefore, to create consistent width buffers on land we first adjusted the perimeter of meadows to match the coastline using digitization tools in ArcGIS.
...
The shoreline modification dataset is available at: *Link to dataset wherever it gets hosted*
...
```{r meadowDiff, fig.pos="H", fig.cap = "The areas of difference between the dataset used in this analysis (created in 2018 for Cristiani et al. 2021) and a newer seagrass spatial dataset from DFO. Only areas where datasets do not overlap are shown."}
knitr::include_graphics(here::here("figures/Seagrass_dataset_differences_1.png"))
```

```{r meadowDiffShorezone, fig.pos="H", fig.cap = "The recent additions to the ShoreZone eelgrass dataset that are not present in the datset used in this study."}
...
5. Trace around the modified area.
    ...
    * The coastaline and polygons we digitized do not always line up perfectly with the imagery depending on the time window used as there is some geographic shifting between imagery of different years (variation in georeferencing of imagery).
